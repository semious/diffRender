...
ParserMng.prototype.getCompleteNode = function () {
	if (this.isSingleTag(this.currentNode)) {
		return "";
	}
	if (this.stat.node.length > 0) {
		var node = this.stat.node[0];
		if (node.name && node.name.charAt(0) === "/" && node.name.substr(1, node.name.length) === this.currentNode.name) {
			this.stat.node.pop();
			this.stat.tagStack.pop();
			return "";
		} else {
			return this._parser.getOuterHTML(this.currentNode, node);
		}
	}
};
